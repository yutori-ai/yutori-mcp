...
def _background_opening(app: str) -> str:
    return (
        ...
        "Wait for a window, select another app, or ask the user to open one; do not repeatedly retry menu or coordinate actions. "
        "When a window exists, get_app_state includes the menu elements exposed by its accessibility snapshot. "
        "Use invoke_app_menu with one observed path, then read fresh state for submenu items. "
        "Unavailable or stale menu targets are refused without foreground fallback. Menu tools never request activation, "
...
async def _prepare_target(computer: MacOSComputer, request: dict[str, Any]) -> dict[str, Any]:
    ...
    Background runs target an app through ``AppSelectingComputer.select_app`` instead
    (see ``app_selection.py``), which owns its own window-target binding.
    """
        ...
        async with AsyncYutoriClient(api_key=api_key, base_url=request["api_base_url"]) as client:
            ...
            if background:
                ...
                async def recover_target() -> int | None:
                    recovered = await _prepare_target(computer, request)
