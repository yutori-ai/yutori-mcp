...
def test_runtime_constants_select_latest_python_surface():
    ...
    assert SDK_VERSION == "0.9.32"
    assert all(len(digest) == 64 for digest in (SDK_ARTIFACT_SHA256, SDK_INSTALLATION_SHA256, SDK_PROVENANCE_SHA256))
    assert '"yutori==0.9.32"' in Path(__file__).parents[1].joinpath("pyproject.toml").read_text()
...
async def test_prepare_target_always_fronts_the_app(monkeypatch):
    ...
    target = await runner_module._prepare_target(computer, request)
